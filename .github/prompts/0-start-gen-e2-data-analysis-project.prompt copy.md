...
│
├── .agents/          # [PHASE 0] Multi-agent orchestration system
│   ├── config.yml    # → Pipeline orchestration settings
│   ├── registry.yml  # → Agent capabilities and skill mappings
│   ├── templates/    # → Agent-specific prompt templates
│   └── skills/       # → Domain-specific knowledge modules
│
├── docs/             # [PHASE 1] Context & Understanding (SHARED)
│   ├── index.md         # → 1: Central documentation hub (navigation)
│   ├── objectives/      # → 2: PROJECT GOALS: What to achieve
│   │   ├── problem_statements/  # → One file per problem statement (ps-{num}-{slug}.md)
│   │   └── user_stories/        # → User stories grouped by problem (ps-{num}/)
...
│   └── project_context/ # → Business objectives, tech stack, folder structure guides
│
...
│   │   │   ├── 1_exploratory/     # → Data profiling, pattern discovery
