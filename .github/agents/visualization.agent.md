...
**ALWAYS place cross-tab metrics outside the `dcc.Tabs` component** — in a persistent global header strip rendered *above* the tab switcher. A metric belongs in the global strip when it: (a) provides top-level context referenced by every tab, (b) is surfaced in ≥ 3 separate tabs, or (c) is cited in the Executive Summary narrative block. Metrics placed inside any single tab become invisible the moment the user switches away; they must not require a tab change to be seen.
...
**Global Metrics Strip** — before defining any tab component list, identify all metrics that are relevant to every tab. Render them once in a dedicated strip placed above the `dcc.Tabs` switcher so they remain visible on every page state:
```
Global Strip (rendered above dcc.Tabs — always visible):
  1. dbc.Row — 3–5 compact KPI badges (value + label + status dot)
  2. Optional: dbc.Row — global filter controls that cascade to all tabs via dcc.Store
     (year-range slider, region / category selector)
```
Criteria for promoting a metric to the global strip:
- It provides orientation context that makes every other tab interpretable (e.g. total population, reporting period, data freshness)
- It is explicitly referenced in the narrative header of ≥ 3 tabs
- It is a primary KPI in the Executive Summary and a supporting reference on all other tabs

Never duplicate a global-strip metric inside individual tab content — reference it visually (e.g. a labelled callout or footnote) rather than re-rendering the same card.
...
**Persistent global metrics strip** — render this *before* `dcc.Tabs` in `app.layout` so it remains visible on every tab:
```python
def _render_global_header(self) -> dbc.Container:
    """
    Persistent strip above the tab switcher.
    Include only metrics that apply to ALL tabs (orientation context, total counts,
    data currency, or primary KPIs cited in ≥3 tab narrative headers).
    Never duplicate metrics that are tab-specific.
    """
    return dbc.Container([
        dbc.Row([
            # One compact badge per global KPI (max 5)
            dbc.Col(dbc.Card(dbc.CardBody([
                html.Span(formatted_value, className="fw-bold fs-5"),
                html.Small(label, className="text-muted d-block"),
                html.Small(comparison_text, style={"color": status_color}),
            ]), className="text-center shadow-sm border-0 bg-light"), xs=6, sm=4, md=2)
            for label, formatted_value, comparison_text, status_color in global_kpis
        ], className="g-2 mb-2"),
        # Optional: global filters that cascade to all tab charts via dcc.Store
        dbc.Row([
            dbc.Col(dcc.RangeSlider(
                ...
                debounce=True,  # required on all RangeSliders
            ), md=8),
        ], className="mb-3") if has_global_filters else html.Div(),
        html.Hr(className="mt-0"),
    ], fluid=True, className="bg-white pt-3 px-3",
       style={"position": "sticky", "top": 0, "zIndex": 1000})

# Layout wiring — global strip sits above the tabs
app.layout = dbc.Container([
    _render_global_header(),      # always visible, regardless of active tab
    dcc.Store(id="global-filter-store"),
    dcc.Tabs(id="main-tabs", value="executive-summary", children=[
        dcc.Tab(label="Executive Summary", value="executive-summary"),
        dcc.Tab(label="Context",           value="context"),
        dcc.Tab(label="Deep Dive",         value="deep-dive"),
        dcc.Tab(label="Forward Looking",   value="forward-looking"),
        dcc.Tab(label="Detail",            value="detail"),
    ]),
    html.Div(id="tab-content"),  # rendered by tab-switch callback
], fluid=True)

# Global filter → store → all tab callbacks read from store
@app.callback(Output("global-filter-store", "data"),
              Input("global-year-slider", "value"),
              prevent_initial_call=True)
def sync_global_filter(year_range):
...
| Layout | Fixed pixel widths on containers; no `dbc.Col` breakpoints — dashboard must be usable on tablet-sized screens; placing cross-tab metrics inside a single tab's content (they become invisible on tab switch — promote to the global strip above `dcc.Tabs`); duplicating a global-strip metric inside individual tab renderers |
...
5c. ✅ Global metrics strip identified and rendered above `dcc.Tabs` — any metric cited in ≥3 tabs or used as cross-tab orientation context is in the strip, not inside an individual tab
5d. ✅ Global strip uses `position: sticky; top: 0; z-index: 1000` so it remains visible on scroll; global filters write to `dcc.Store` and read by all tab callbacks
